...
// LocalStorage-based visitor counter (no Firebase needed)
function initializeVisitorCounter() {
  let visitorCount = parseInt(localStorage.getItem('seatPlanVisitorCount') || '1247');
  
  // Increment visitor count
  visitorCount++;
  localStorage.setItem('seatPlanVisitorCount', visitorCount.toString());
...
function updateCounterDisplay(count) {
  ...
  counters.forEach((counter, index) => {
    ...
    if (span) {
      ...
      setTimeout(() => {
        counter.style.animation = 'counterGlow 1s ease-out';
      }, index * 100);
      
      // Create new digit element
      const newDigit = document.createElement('span');
      newDigit.textContent = digit;
      newDigit.style.transform = 'translateY(100%) scale(0.8)';
      counter.appendChild(newDigit);
      
      const oldSpan = counter.querySelector('span:first-child');
      
      if (oldSpan && oldSpan !== newDigit) {
        oldSpan.style.transform = 'translateY(-100%) scale(1.2)';
      }
      
      requestAnimationFrame(() => {
        newDigit.style.transform = 'translateY(0) scale(1)';
      });
...
document.addEventListener('DOMContentLoaded', () => {
  // Set initial display before incrementing
  const currentCount = parseInt(localStorage.getItem('seatPlanVisitorCount') || '1247');
  const counters = document.querySelectorAll('.counter');
  const countStr = currentCount.toString().padStart(5, '0');
